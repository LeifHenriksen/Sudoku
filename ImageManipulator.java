...
public class ImageManipulator {
    ...
    private static Mat loadAndPreprocess(String file) {
        ...
        if(s.empty())
            Log.d("LoadAndProcess", "image videeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee");
        Mat outerBox = new Mat(s.size(), CV_8UC1);

        int blockSize = (int) (s.size().height * 0.05d);
        blockSize = blockSize%2==0 ? blockSize+1 : blockSize;

        Imgproc.adaptiveThreshold(s, outerBox, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, blockSize, 10);
